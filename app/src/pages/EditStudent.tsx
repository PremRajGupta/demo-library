...
export default function EditStudent() {
  ...
  const [admissionResult] = useState<any>(null);
    ...
    if (id) {
      studentApi.getStudentById(id).then(student => {
        ...
        setFormData({
          ...
          customShiftHours: student.customShiftHours ? String(student.customShiftHours) : '',
          joiningDate: student.joiningDate ? student.joiningDate.split('T')[0] : toDateInputValue(),
        ...
        if (student.aadharBack) setAadharBackPreview(student.aadharBack);
        if (student.seatNumber) {
          setSeatSearch(student.seatNumber);
          setInitialSeat(student.seatNumber);
        }
      }).catch(err => {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      formData.seatNumber !== initialSeat
    ) {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg">
            <div className="flex flex-col sm:flex-row sm:items-start gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-semibold text-[#0f172a]">Edit Student Record</h2>
                <p className="text-sm text-[#64748b] mt-1">Update student profile details, assign seat & fees</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#64748b] mt-1">
                    {isOtherShift
                      ? 'Other shift selected.'
                      : isPresetShift
                        ? 'Preset shift selected.'
                        : 'Select a time shift.'}
                  </p>
                    ...
                    <div className="absolute z-20 mt-1 w-full max-h-52 overflow-y-auto rounded-lg border border-[#e2e8f0] bg-white shadow-lg">
                      {filteredSeats.map((seat) => (
                        <button
                          ...
                          className={`w-full px-4 py-2.5 text-left text-sm hover:bg-[#f1f5f9] transition-colors ${
                            formData.seatNumber === seat ? 'bg-[#eff6ff] text-[#0369a1] font-medium' : 'text-[#1e293b]'
                          }`}
                        >
                          {seat}
                        </button>
                      ...
                      {showOtherSeatOption && (
                        <button
                          ...
                          className={`w-full px-4 py-2.5 text-left text-sm border-t border-[#e2e8f0] hover:bg-[#f1f5f9] transition-colors ${
                            formData.seatNumber === 'other' ? 'bg-[#eff6ff] text-[#0369a1] font-medium' : 'text-[#64748b]'
                          }`}
                        >
                          Other (custom seat)
                        </button>
